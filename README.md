...
The content is original and researched, but the Uniform Securities Act is model legislation and adopting states differ, some figures drift, and a few boundaries are easy to state too broadly. Where that is true the point is flagged rather than asserted flatly.

<!-- review-counts:start (generated by `npm run review:md` — do not edit by hand) -->
**12 items are currently flagged**, spanning 12 questions across 8 chapters.
<!-- review-counts:end -->

Each entry records what the app tells a student, why it was flagged, and the specific thing to check — alongside the question ids that would have to change with the rule, because a chapter fix that leaves the questions alone is worse than no fix at all. Both [`REVIEW.md`](./REVIEW.md) and the in-app `/review` page render from `src/data/reviewItems.ts`, so the two cannot drift apart.
...
│   ├── reviewItems.ts           # content flagged for review (see above)
