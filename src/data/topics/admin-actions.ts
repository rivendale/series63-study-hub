...
**If other material tells you the 1956 act is "2 years after the contract of sale", it is not wrong — it is quoting the ORIGINAL 1956 text**, where the limitations clause sat at **§410(e)**. NASAA's amendments moved it to **§410(f)** and widened it to the 3-and-2 pair above, inserting a new §410(e) for survival of actions. The 2002 act's own prefatory note still describes the 1956 act by its original figure, which is why the two-year version circulates. **The exams are written against the amended edition, so use §410(f).**

Note that the **2002 act splits by claim type where the 1956 act does not.** A claim resting on a **registration** violation gets **1 year from the violation** under §509(j)(1) — flat, with no discovery extension, the statute simply sets a flat period for that branch. The 2-and-5 pair under §509(j)(2) covers the **fraud** side. If a question about the 2002 act says "the **earlier** of", it is describing that branch. Treat these two as the branches the exam actually tests rather than a complete map of §509 — the section carries further categories, and an adopting state may draw the lines differently.

Wherever a period has **two** triggers — the 1956 rule and the 2002 **fraud** branch, so everything above except §509(j)(1) — the structure is what the question turns on: the clock runs from the **earlier** of a **conduct** trigger and a **discovery** trigger, which means discovery can only **shorten** the window, never extend it. A buyer who discovers the violation four years after the sale is out of time under the 1956 act even though the discovery was yesterday; a buyer who spotted it on day one has **2** years, not 3. "Whichever first occurs" is doing the work — a plaintiff cannot revive a stale claim by pleading late discovery. Under **§509(j)(1)** there is nothing to compare: one year from the violation, full stop.
...
**Criminal** cases run on their own clock — **5 years** after the alleged violation. Note that in the model text this clause is **bracketed**, which in that document marks text a state supplies or omits: the official comment treats it as optional for states that already have a general criminal limitations period. So it is the figure to know, but it is not universal the way the civil rule is. Do not carry the civil figure across to the criminal side.
...
Every registrant files an **irrevocable consent** appointing the Administrator as agent for service of process for any action under the USA.`,
    ...
    keyTerms: [
      ...
      { term: 'SOL', definition: '1956 act §410(f) — the earlier of 3 years from the contract of sale or the rendering of investment advice, or 2 years from discovery. Not "3 years from the violation": the trigger is the sale or the advice. 2002 act splits by claim type — §509(j)(1) registration: 1 year from the violation, flat; §509(j)(2) fraud: earlier of 2 years from discovery or 5 years from the violation.' },
