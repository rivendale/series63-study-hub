  ...
  return (
    <article className="max-w-3xl mx-auto">
      <div className="sticky top-0 -mx-4 md:-mx-6 px-4 md:px-6 py-3 bg-slate-50/95 dark:bg-slate-950/95 backdrop-blur z-20 border-b border-slate-200 dark:border-slate-800">
        <Link to="/curriculum" className="inline-flex items-center gap-1 text-sm text-slate-600 dark:text-slate-400 hover:text-blue-600">
          <ArrowLeft className="w-4 h-4" />
          Curriculum
        </Link>
        <div className="mt-2 h-1 w-full bg-slate-200 dark:bg-slate-800 rounded-full overflow-hidden">
          <div className="h-full bg-blue-600 transition-all" style={{ width: `${scrollPct}%` }} />
        </div>
      </div>

      <header className="mt-4">
        <div className="flex items-center gap-2 text-sm">
          <span className="font-semibold text-blue-600 dark:text-blue-400">{topic.weight}</span>
          <span className="text-slate-400">&middot;</span>
          <span className="text-slate-600 dark:text-slate-400">Topic {topic.order} of {topics.length}</span>
        </div>
        <h1 className="mt-1 text-3xl font-bold text-slate-900 dark:text-slate-100">{topic.title}</h1>
        ...
        <section className="mt-6 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5">
          ...
          <dl className="mt-3 space-y-3">
            {topic.keyTerms.map((kt, i) => (
              <div key={i}>
                <dt className="font-semibold text-slate-900 dark:text-slate-100">{kt.term}</dt>
                <dd className="text-sm text-slate-600 dark:text-slate-400 mt-0.5">{kt.definition}</dd>
              </div>
            ))}
          </dl>
        </section>
      ...
      <div className="mt-8 flex flex-col sm:flex-row gap-3">
        ...
        <Link to={`/quiz/topic/${topic.id}`} className="inline-flex items-center justify-center gap-2 px-4 py-3 rounded-lg font-semibold bg-blue-600 text-white hover:bg-blue-700 transition min-h-[44px]">
